...
// @failure 400
// @failure 500
func (dc *SearchController) HybridSearch(c *gin.Context) {
	var req dto.SearchRequest
	if err := c.ShouldBindQuery(&req); err != nil {
		respondWithError(c, err)
		return
	...
	chunks, err := dc.srv.HybridSearch(
		c.Request.Context(),
		req.Workspace,
		req.Query,
		req.Limit,
...
// HybridSearchDocuments godoc
//
// @summary Hybrid search (documents)
// @router /search/hybrid/docs [get]
// @tags search
// @param q query string true "Search query" minlength(1)
// @param workspace query string true "Workspace ID"
// @param limit query int false "Search result limit" minimum(1) default(5)
...
// @failure 400
// @failure 500
func (dc *SearchController) HybridSearchDocuments(c *gin.Context) {
	var req dto.SearchRequest
	if err := c.ShouldBindQuery(&req); err != nil {
		respondWithError(c, err)
		return
	...
	documentIDs, err := dc.srv.HybridSearchDocuments(
		c.Request.Context(),
		req.Workspace,
		req.Query,
		req.Limit,
